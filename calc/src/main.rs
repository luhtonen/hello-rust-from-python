// Web microservice for calculating multiple type of operations
use actix_web::{get, web, App, HttpResponse, HttpServer, Responder};

#[get("/")]
async fn index() -> impl Responder {
    HttpResponse::Ok().body("This is calculator microservice")
}
...
#[get("/devide/{a}/{b}")]
async fn devide(info: web::Path<(i32, i32)>) -> impl Responder {
    let res = calc::devide(info.0, info.1);
    HttpResponse::Ok().body(res.to_string())
}
