...
fn main() {
    ...
    match args.command {
        Some(Commands::Classify { file }) => {
            ...
            let lyrics = sqlite_hf::read_lyrics(&file);
            let result = sqlite_hf::classify_lyrics(lyrics);
        ...
        Some(Commands::Candidates {}) => {
            for candidate in sqlite_hf::get_all_zeroshotcandidates() {
                println!("{}", candidate);
            }
        },
        Some(Commands::Lyrics {file}) => {
            println!("Lyrics {}", file);
            for line in sqlite_hf::read_lyrics(&file) {
                println!("{}", line);
            }
        },
        None => {
