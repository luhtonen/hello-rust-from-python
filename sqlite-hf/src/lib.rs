/// Hugging Face Rust library which analyses lyrics of the songs and puts them into a sqlite database.
...
fn create_db() -> sqlite::Connection {
    ...
    db.execute("INSERT INTO zeroshotcandidates (label) values('pop')")
        .unwrap();
    db.execute("INSERT INTO zeroshotcandidates (label) values('hip pop')")
        .unwrap();
...
// return all zero shot classification candidates as a vector of string
...
/// Accepts a vector of strings as lyrics and grabs candidates from the memory sqlite database.
pub fn classify_lyrics(lyrics: Vec<String>) -> Vec<Vec<Label>> {
    // extract candidate labels from sqlite database put in an array
    ...
    // classify lyrics
    zero_shot_model
        .predict_multilabel([lyrics], candidate_labels, None, 128)
        .unwrap()
}
